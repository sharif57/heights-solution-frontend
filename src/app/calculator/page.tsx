...
export default function FullCalculator() {
  ...
  const [insuranceSG, setInsuranceSG] = useState(initialData.insuranceSG);
  const [leadGenCost, setLeadGenCost] = useState(initialData.leadGenCost);
  const [insuranceDist, ] = useState(initialData.insuranceDist);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 bg-white rounded-lg shadow-md p-6">
          ...
          <input
            ...
            className="w-full sm:w-64 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
            placeholder="Enter practice size"
          />
        </div>
        ...
        <div className="mb-8 bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { label: "Treatment Weeks", value: treatmentWeeks, setter: setTreatmentWeeks },
              { label: "Wound Shrinkage (%)", value: woundShrinkage, setter: setWoundShrinkage },
              { label: "Treatment Cancellation (%)", value: txtCancellation, setter: setTxtCancellation },
              { label: "Materials Cost (% of UCR)", value: materialsCost, setter: setMaterialsCost },
              { label: "Billing Expense (%)", value: billingExp, setter: setBillingExp },
              { label: "Insurance SG&A (%)", value: insuranceSG, setter: setInsuranceSG },
              { label: "Lead Gen Cost (%)", value: leadGenCost, setter: setLeadGenCost },
            ].map((input, idx) => (
              <div key={idx}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {payorTypes.map((payor, i) => {
            ...
            return (
              ...
              >
                <h2 className="text-xl font-semibold text-blue-600 mb-4">{payor}</h2>
                <div className="space-y-3">
                  {[
                    { label: "Payor %", value: payorPercentages[i], type: "payor" },
                    { label: "DFU %", value: dfuPercentages[i], type: "dfu" },
                    { label: "Qualifying %", value: qualifyingPercentages[i], type: "qual" },
                    { label: "Wound Size (cm²)", value: woundSizes[i], type: "wound" },
                    { label: "UCR ($)", value: ucr[i], type: "ucr" },
                    { label: "Commission %", value: commissionPercentages[i], type: "commission" },
                    { label: "CC Fees (%)", value: ccFees[i], type: "ccFees" },
                  ].map((input, idx) => (
                    <div key={idx}>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {input.label}
                      ...
                      <input
                        type="number"
                        value={input.value}
                        onChange={(e) => updateValue(input.type, i, Number(e.target.value))}
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                        placeholder={`Enter ${input.label.toLowerCase()}`}
                      />
                    </div>
                  ...
                  <div className="pt-4 space-y-2 text-sm text-gray-600">
                    <p><span className="font-medium">Patients:</span> {values.patientCount}</p>
                    <p><span className="font-medium">DFU Count:</span> {values.dfuCount}</p>
                    <p><span className="font-medium">Qualifying:</span> {values.qualifying}</p>
                    <p><span className="font-medium">Total Units:</span> {values.totalUnits}</p>
                    <p><span className="font-medium">Provider Profit:</span> ${values.providerProfit}</p>
                    <p><span className="font-medium">Distributor Profit:</span> ${values.distributorProfit}</p>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Summary Table</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full border border-gray-200 text-sm">
              <thead className="bg-gray-100">
                <tr>
                  <th className="p-3 text-left font-medium text-gray-700 border-r border-gray-200">Category</th>
                  <th className="p-3 text-left font-medium text-gray-700 border-r border-gray-200">Total</th>
                  {payorTypes.map((payor, i) => (
                    <th key={i} className="p-3 text-left font-medium text-gray-700 border-r border-gray-200">{payor}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  { label: "% of Patient Base", total: "100%", values: payorPercentages.map(p => `${p.toFixed(1)}%`) },
                  { label: "Unique Pats / Yr", total: practiceSize, values: payorPercentages.map(p => ((p / 100) * practiceSize).toFixed(0)) },
                  { label: "% DFU Patients", total: ((totalDFU / practiceSize) * 100).toFixed(1) + "%", values: dfuPercentages.map(dfu => `${dfu.toFixed(1)}%`) },
                  { label: "Qualifying Patients", total: totalQualifying.toFixed(1), values: payorTypes.map((_, i) => calculate(i).qualifying) },
                  {
                    label: "% Qualifying of DFU",
                    total: totalDFU > 0 ? ((totalQualifying / totalDFU) * 100).toFixed(1) + "%" : "0",
                    values: payorTypes.map((_, i) => {
                      const dfu = Number(calculate(i).dfuCount);
                      const qual = Number(calculate(i).qualifying);
                      return dfu > 0 ? ((qual / dfu) * 100).toFixed(1) + "%" : "0.0%";
                    })
                  },
                  { label: "Total Units", total: payorTypes.reduce((acc, _, i) => acc + Number(calculate(i).totalUnits), 0), values: payorTypes.map((_, i) => calculate(i).totalUnits) },
                  { label: "Provider Profit ($)", total: totalProviderProfit.toFixed(0), values: payorTypes.map((_, i) => calculate(i).providerProfit) },
                  { label: "Distributor Profit ($)", total: totalDistributorProfit.toFixed(0), values: payorTypes.map((_, i) => calculate(i).distributorProfit) },
                  { label: "Lead Gen Sales Commission ($)", total: totalLeadGenSalesCommission.toFixed(0), values: payorTypes.map((_, i) => calculate(i).leadGenSalesCommission) },
                ].map((row, idx) => (
                  <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50 transition-colors duration-150">
                    <td className="p-3 font-medium text-gray-700 border-r border-gray-200">{row.label}</td>
                    <td className="p-3 text-gray-600 border-r border-gray-200">{row.total}</td>
                    {row.values.map((value, i) => (
                      <td key={i} className="p-3 text-gray-600 border-r border-gray-200">{value}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
